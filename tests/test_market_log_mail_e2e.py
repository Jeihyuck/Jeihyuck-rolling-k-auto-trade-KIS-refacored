...
def fixture_repo(tmp_path: Path, market: str, *, smtp_mode: str = "dry") -> Path:
    ...
    if market == "kr":
        paths = (f"reports/kr_prep/{DATE}", f"runtime/kr/watchlist/{DATE}", f"runtime/kr/session/{DATE}", "bot_state/trader_ledger")
    else:
...
def run_mail(root: Path, market: str, *, dry=True):
    ...
    env={**os.environ,"NULLIM_KST_RUN_DATE":DATE,"US_TRADE_DATE":DATE,"NULLIM_LOG_MAIL_OUT":str(archive),"NULLIM_KEEP_MAIL_ARCHIVE":"1","MAIL_TO":"ops@example.test","NULLIM_SMTP_RETRY_SLEEP_1":"0","NULLIM_SMTP_RETRY_SLEEP_2":"0"}
...
def test_kr_end_to_end_uses_canonical_paths_and_dry_run_isolated(tmp_path):
    ...
    with tarfile.open(archive,"r:gz") as tf:
        ...
        assert any("bot_state/trader_ledger" in name for name in names)
...
def test_final_health_requires_real_mail_and_returns_matching_exit_code(tmp_path):
    ...
    missing=subprocess.run(["bash",str(scripts/"check-nullim-day-health.sh"),"kr",DATE],cwd=root)
    ...
    dry=subprocess.run(["bash",str(scripts/"check-nullim-day-health.sh"),"kr",DATE],cwd=root)
    ...
    success=subprocess.run(["bash",str(scripts/"check-nullim-day-health.sh"),"kr",DATE],cwd=root)
