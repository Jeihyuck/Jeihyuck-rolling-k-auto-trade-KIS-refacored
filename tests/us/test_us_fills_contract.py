...
def test_get_us_fills_today_preserves_real_tr_cont_header_and_fetches_next_page(monkeypatch):
    from trader.us.execution.kis_us_client import KisUSClient
    ...
    responses = iter([
        ...
        FakeResp({"rt_cd": "0", "output": [{"odno": "2"}]}, "D"),
    ])

    def fake_get(url, headers=None, params=None, timeout=None, **kwargs):
        calls.append((dict(headers or {}), dict(params or {})))
        return next(responses)

    monkeypatch.setattr("requests.get", fake_get)
    client = KisUSClient(env="practice")
    monkeypatch.setattr(client, "get_access_token", lambda: "TOKEN")
    monkeypatch.setattr(client, "_apply_rate_limit", lambda *_: None)
...
def test_get_us_fills_today_follows_cursor_when_response_header_missing(monkeypatch):
    from trader.us.execution.kis_us_client import KisUSClient
...
def test_get_us_fills_today_rejects_unknown_continuation_status(monkeypatch):
    from trader.us.execution.kis_us_client import KisUSClient, KisUSClientError
    ...
    with pytest.raises(KisUSClientError, match="unknown tr_cont"):
        client.get_us_fills_today("2026-05-05")
