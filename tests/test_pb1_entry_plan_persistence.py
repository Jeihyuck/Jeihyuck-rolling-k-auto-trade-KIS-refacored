...
def test_latest_buy_plan_search_skips_newer_rows_without_plan():
    engine = create_engine("sqlite:///:memory:")
    schema = schema_for_engine(engine)
    schema.metadata.create_all(engine)
    plan = build_entry_exit_plan(
    ...
    orders.create_intent_idempotent(env="practice", run_id=None, strategy="pb1_pullback_close", sid=1, mode=1, code="005930", market="KOSPI", side="BUY", ord_type="LIMIT", qty=1, limit_price=10000, stage="TEST", client_order_key="older-with-plan", request_json={"entry_exit_plan": plan, "entry_meta": {}}, status="SUBMITTED")
    orders.create_intent_idempotent(env="practice", run_id=None, strategy="pb1_pullback_close", sid=1, mode=1, code="005930", market="KOSPI", side="BUY", ord_type="LIMIT", qty=1, limit_price=10000, stage="TEST", client_order_key="newer-without-plan", request_json={"entry_meta": {}}, status="SUBMITTED")
    with engine.begin() as conn:
        conn.execute(sa.update(schema.orders).where(schema.orders.c.client_order_key == "older-with-plan").values(created_at=datetime(2026, 6, 10, 9, 0, 0)))
        conn.execute(sa.update(schema.orders).where(schema.orders.c.client_order_key == "newer-without-plan").values(created_at=datetime(2026, 6, 10, 9, 1, 0)))
    found = orders.find_latest_buy_entry_exit_plan("practice", "pb1_pullback_close", "005930", lookback=5)
    assert found["entry_exit_plan"]["policy_version"] == "pb1_entry_exit_plan_v1"
