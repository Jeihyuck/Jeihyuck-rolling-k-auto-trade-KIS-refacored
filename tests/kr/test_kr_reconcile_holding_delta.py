...
def _open_buy_order(*, pre_holding: int | None, submitted_qty: int, qty: int = 1, status: str = "ACKED") -> dict:
...
def test_unresolved_buy_reconciles_to_fill_from_authoritative_holding_delta() -> None:
    orders_repo = MagicMock()
    fills_repo = MagicMock()
    orders_repo.get_open_orders.return_value = [
        _open_buy_order(pre_holding=0, submitted_qty=1, qty=1, status="UNRESOLVED_ACK")
    ]
    holdings_rows = [{"pdno": "090430", "hldg_qty": "1", "pchs_avg_pric": "10000"}]
    ...
    assert orders_repo.upsert_reconciled_order.call_args.kwargs["status"] == "FILLED"
    assert fills_repo.upsert_fill.call_args.kwargs["qty"] == 1
