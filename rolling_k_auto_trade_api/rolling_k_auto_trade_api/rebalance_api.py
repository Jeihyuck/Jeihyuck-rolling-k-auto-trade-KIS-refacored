...
def rebalance_backtest_monthly(
    ...
    print(
        f"[DEBUG] User-Agent='{ua}', Accept='{accept}', Referer='{referer}', is_curl={is_curl}"
    )
    ...
    try:
        ...
        return JSONResponse(
            content={"error": "날짜 형식 오류: YYYY-MM-DD 형식을 사용하세요."}
        )
    ...
    try:
        ...
        for i in range(len(periods) - 1):
            ...
            print(
                f"[DEBUG] {i+1}번째 월 리밸런싱: train {start_train}~{end_train}, test {start_test}~{end_test}"
            )
            selected = []
        ...
        print(
            f"[SUCCESS] final_df 생성 완료, 총 {len(final_df)}개 종목 → 파일: {filename}"
        )
        ...
        return JSONResponse(content={"error": f"백테스트 예외 발생: {str(e)}"})
