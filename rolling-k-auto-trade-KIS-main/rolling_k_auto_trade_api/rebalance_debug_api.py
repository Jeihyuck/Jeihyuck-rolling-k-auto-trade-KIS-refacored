...
def debug_backtest_monthly(
    ...
    try:
            ...
            for code, name in tickers:
                try:
                    ...
                    if (final_ret > 0) and (win_rate > 0.2) and (mdd > -30):
                        ...
                        debug_logs.append(f"{name}({code}): 필터 탈락 (수익률 {final_ret:.2f}%, 승률 {win_rate*100:.1f}%, MDD {mdd:.1f}%)")
        ...
        return {
            "결과": pd.concat(all_results, ignore_index=True).to_dict(orient="records") if all_results else [],
            "로그": debug_logs
        }
