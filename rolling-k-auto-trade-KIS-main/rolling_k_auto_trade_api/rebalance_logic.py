...
def run_rebalance_strategy():
    ...
    for _, row in df.iterrows():
        ...
        if float(result["mdd_pct"].replace("%", "")) <= 10 and float(result["win_rate_pct"].replace("%", "")) >= 70:
            results.append(result)

    return sorted(results, key=lambda x: float(x["avg_return_pct"].replace("%", "")), reverse=True)[:5]
