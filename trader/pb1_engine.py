...
from trader.kr.pb1.exit_planning import (
    resolve_core_trend_follow_exit as resolve_core_trend_follow_exit_impl,
    resolve_day_protect_exit as resolve_day_protect_exit_impl,
    resolve_exit_policy_wrapper as resolve_exit_policy_impl,
    resolve_swing_staged_exit as resolve_swing_staged_exit_impl,
)
...
def _resolve_day_protect_exit(
    ...
    *,
    ...
    return resolve_day_protect_exit_impl(
        pos,
        mark,
        now_hhmm,
        ret_pct=ret_pct,
...
def _resolve_swing_staged_exit(
    ...
    *,
    ...
    return resolve_swing_staged_exit_impl(
        pos,
        mark,
        ma20,
        ret_pct=ret_pct,
        ...
        highest_ret_pct=highest_ret_pct,
    )
...
def _resolve_core_trend_follow_exit(
    ...
    *,
    ...
    return resolve_core_trend_follow_exit_impl(
        pos,
        mark,
        ma20,
        ma50,
...
def _resolve_exit_policy(
    *,
    ...
    return resolve_exit_policy_impl(
        days_held=days_held,
...
class PB1Engine:
    ...
    def _resolve_entry_identity_from_mapping(self, source: dict[str, Any] | None) -> dict[str, str]:
        return resolve_entry_identity_from_mapping(source)
    ...
    def _resolve_buy_cooldown_state(
        ...
        *,
        ...
        return resolve_buy_cooldown_state(
            today=self._today,
            code=code,
            cooldown_until=cooldown_until,
            holding_qty=holding_qty,
            today_buy_exists=today_buy_exists,
            today_fill_exists=today_fill_exists,
            cooldown_source_events_count=cooldown_source_events_count,
            last_fill_event_at=last_fill_event_at,
            cooldown_source=cooldown_source,
            recent_valid_exit_event=recent_valid_exit_event,
            recent_exit_reason=recent_exit_reason,
        )
    ...
    def _resolve_entry_capital(
        ...
        *,
        ...
        return resolve_entry_capital(
            ...
            paper_max_capital_krw=PAPER_MAX_CAPITAL_KRW,
            cap_cap=CAP_CAP,
        )
        ...
        for cf in ranked:
            ...
            if source and source != "ask":
                ...
                if source == "daily_close":
                    cf.features["price_source"] = "daily_close_fallback"
                    cf.features["price_gate_blocked"] = True
            cf.features["order_price"] = float(order_px)
            cf.features["order_price_source"] = source or "unknown"
            cf.features.setdefault("price_source", source or "unknown")
    ...
    def _pretrade_check(
        ...
        *,
        ...
        if gate_reasons:
            ...
            try:
                self._append_ledger_event(
                    event_type="ORDER_SKIP", code=code, market=market, mode=mode, side=side, qty=qty, price=price,
                    client_order_key=client_order_key, ok=False, reasons=[f"pretrade:{reason}" for reason in gate_reasons],
                    stage=stage,
                )
            except Exception:
                logger.exception("[PB1][LEDGER][PRETRADE_GATE_SKIP_FAIL] code=%s", self._display_code(code))
            return False
        if not self.kis:
            return False
    ...
    def _place_entry(self, cf: CandidateFeature) -> dict[str, int | str]:
        ...
        if not isinstance(plan, dict):
            ...
            stop_price_for_plan = float(cf.features.get("stop_price") or cf.features.get("initial_stop") or 0.0)
            trigger_info_for_plan = cf.features.get("entry_trigger") if isinstance(cf.features.get("entry_trigger"), dict) else {}
        ...
        qty = int(plan.get("qty") or cf.planned_qty or 0)
        entry_price = float(plan.get("entry_price") or 0.0)
        limit_price = float(plan.get("limit_price") or plan.get("order_price") or entry_price or 0.0)
        ...
        try:
            self._append_ledger_event(
                ...
                stage=stage,
                payload_json={"entry_plan": plan, "features": cf.features, "entry_meta": entry_meta, "entry_exit_plan": entry_exit_plan_dict, "trace_id": entry_meta.get("trace_id")},
            )
        except Exception:
        ...
        self._append_ledger_event(
            ...
            reasons=["submit"],
            stage=stage,
            payload_json={"entry_plan": plan, "entry_meta": entry_meta, "entry_exit_plan": entry_exit_plan_dict, "trace_id": entry_meta.get("trace_id"), "kis_odno": kis_odno},
        )
    ...
    def _place_entry_close(self, cf: CandidateFeature) -> dict[str, int | str]:
        ...
        return submit_entry_buy_order(
            ...
            entry_meta=entry_meta,
            entry_exit_plan_dict=entry_exit_plan_dict,
            gate_snapshot=gate_snapshot,
            status=status,
        )
        ...
        if self.phase in {"prep", "entry", "pm_entry"} and not skip_entry_scan:
            ...
            with self._stage_timer("entry.buyable_gate"):
                for cf in ok_after_risk:
                    ...
                    cf.features["initial_stop"] = float(stop0)
                    cf.features["stop_price"] = float(stop0)
                    actual_stage = self._entry_stage_name()
                    order_price_for_plan = float(order_price or entry_price or 0.0)
                    cf.entry_plan = self._build_entry_plan(
                        cf,
                        entry_price=float(entry_price),
                        order_price=order_price_for_plan,
                    ...
                    logger.info(
                        ...
                        float(cf.entry_plan.get("order_price") or 0.0), float(cf.entry_plan.get("stop_price") or 0.0),
                        plan_reasons or ["ok"],
                    )
                    if not plan_ok:
