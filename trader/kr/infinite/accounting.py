...
def apply_confirmed_fill(state: State, intent: OrderIntent, broker: BrokerOrderState,
    ...
    if delta_qty > 0 and intent.side in {"SELL_PARTIAL", "SELL_ALL"}:
        ...
        metadata = {**state.metadata, "last_profit_fill_qty": broker.filled_qty,
                    "last_profit_fill_date": trade_date.isoformat()}
        if fully_filled:
            filled_stage = pending.removesuffix("_SUBMITTED") + "_FILLED" if pending else ""
            metadata["pending_profit_stage"] = None
            if filled_stage:
                metadata["profit_stage"] = filled_stage
        else:
            # Partial broker evidence must keep the submitted fence active.  It
