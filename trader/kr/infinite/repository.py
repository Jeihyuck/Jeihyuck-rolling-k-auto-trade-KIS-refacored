...
class InfiniteRepository:
    ...
    def load_state(self) -> State | None:
        with self.engine.connect() as conn:
            row = conn.execute(
                text("SELECT * FROM kr_infinite_state WHERE trading_epoch_id=:epoch AND strategy_id='KR_INFINITE_V1' AND symbol='122630'"),
                {"epoch": self._epoch()},
            ).mappings().first()
        if row is None:
            return None
    ...
    def intent_keys(self) -> frozenset[str]:
        with self.engine.connect() as conn:
            rows = conn.execute(
                text("SELECT idempotency_key FROM kr_infinite_order_intents WHERE trading_epoch_id=:epoch"),
                {"epoch": self._epoch()},
            ).all()
    ...
    def pending_intents(self) -> list[OrderIntent]:
        with self.engine.connect() as conn:
            rows = conn.execute(text("""SELECT id,cycle_id,trade_date,side,idempotency_key,requested_qty,unit_sequence,
                ...
                WHERE trading_epoch_id=:epoch AND status=ANY(:statuses) ORDER BY id"""),
                {"epoch": self._epoch(), "statuses": list(PENDING)}).mappings().all()
    ...
    def create_intent(self, state: State, decision: Decision, trade_date: date, market_state: str) -> bool:
        ...
        with self.engine.begin() as conn:
            row = conn.execute(text("""INSERT INTO kr_infinite_order_intents(
                ...
                VALUES(:epoch,'KR_INFINITE_V1','122630',:cycle,:day,:side,:reason,:seq,:notional,:qty,:price,:key,:market,CAST(:metadata AS jsonb))
                ON CONFLICT(idempotency_key) DO NOTHING RETURNING id"""), {
                "epoch": self._epoch(), "cycle": state.cycle_id, "day": trade_date, "side": decision.action.value,
    ...
    def mark_submitted(self, key: str, broker_order_id: str | None) -> None:
        with self.engine.begin() as conn:
            conn.execute(text("""UPDATE kr_infinite_order_intents SET broker_order_id=:order_id,
                status='SUBMITTED',updated_at=NOW() WHERE trading_epoch_id=:epoch AND idempotency_key=:key"""),
                {"epoch": self._epoch(), "order_id": broker_order_id, "key": key})
    ...
    def mark_rejected(self, key: str, reason: str) -> None:
        with self.engine.begin() as conn:
            conn.execute(text("""UPDATE kr_infinite_order_intents SET status='REJECTED',
                ...
                WHERE trading_epoch_id=:epoch AND idempotency_key=:key"""),
                {"epoch": self._epoch(), "key": key, "metadata": json.dumps({"rejection": reason})})
        ...
        with self.engine.begin() as conn:
            for intent, broker in updates:
                conn.execute(text("""UPDATE kr_infinite_order_intents SET status=:status,filled_qty=:qty,
                    filled_notional_krw=:notional,filled_avg_price=:average,updated_at=NOW()
                    WHERE trading_epoch_id=:epoch AND id=:id"""),
                    {"epoch": self._epoch(), "status": broker.status, "qty": broker.filled_qty, "notional": broker.filled_notional_krw,
    ...
    def _save_state(self, conn, state: State) -> None:
