...
def _post_pb1_tick_reconcile(engine_obj: Any) -> None:
    ...
    if not force_refresh:
        ...
        if isinstance(tick_snapshot, dict):
            snapshot = tick_snapshot
            balance_source = "pb1_tick_snapshot"
    ...
    if snapshot is None:
        ...
        if hasattr(kis, "get_balance_cached"):
            ...
            balance_source = "kis_forced" if force_refresh else "kis_cached_or_fetch"
        elif hasattr(kis, "get_balance"):
            snapshot = kis.get_balance()
            balance_source = "kis_get_balance"

    logger.info(
        "[KR_BROKER_TRUTH][POST_TICK][BALANCE_SOURCE] api_submitted=%s force_refresh=%s source=%s",
        api_submitted,
        int(force_refresh),
        balance_source,
    )
