"""Runtime guards for legacy PB1 session finalization and safe KR buyable backfill.

This module intentionally keeps compatibility shims out of ``trader.__init__``.
The actual long-term fix should still be to remove the legacy unqualified
``engine_runner`` lookup from ``pb1_runner.py``.  Until then, these guards are
small, idempotent, and fail-closed so the trade loop is not crashed by guard
code itself.
"""
...
class _MissingEngineRunnerSummary:
    """Last-resort empty summary before the first real PB1 engine tick exists."""
...
def _buyable_backfill_block_reason(
    engine: Any,
    cf: Any,
    *,
...
def _candidate_safe_for_buyable_backfill(cf: Any) -> tuple[bool, str]:
    ...
    atr_ratio = _normalize_ratio(
        features.get("atr_pct") if features.get("atr_pct") is not None else features.get("atr_percent")
    )
    max_atr = _normalize_ratio(
        os.getenv("PB1_BUYABLE_BACKFILL_MAX_ATR_PCT", os.getenv("PB1_MAX_ATR_PCT", "10")),
        0.10,
    )
    ...
    score = _safe_float(
        features.get("score_final") or features.get("final_score") or features.get("score")
    )
    min_score = _env_float("PB1_BUYABLE_BACKFILL_MIN_SCORE", 30.0)
    style = str(
        features.get("entry_style_selected")
        or features.get("entry_component")
        or features.get("entry_signal")
        or ""
    ).strip()
    ...
    if not setup_like_ok:
        ...
        if price < ma20 * _env_float("PB1_BUYABLE_BACKFILL_MIN_CLOSE_MA20_RATIO", 0.92):
            return False, "close_too_far_below_ma20"
        if price < ma50 * _env_float("PB1_BUYABLE_BACKFILL_MIN_CLOSE_MA50_RATIO", 0.85):
...
def _ensure_backfill_entry_plan(engine: Any, cf: Any, *, price: float, qty: int) -> bool:
    ...
    if not getattr(cf, "client_order_key", None) and hasattr(engine, "_client_order_key"):
        try:
            ...
            _logger.info(
                "[ENTRY][BUYABLE_BACKFILL][KEY_SKIP] code=%s err=%s",
                _candidate_code(cf),
                exc,
            )
            return False
    ...
    if hasattr(engine, "_build_entry_plan"):
        try:
            ...
            if hasattr(engine, "_validate_entry_plan"):
                ...
                if not ok:
                    _logger.info(
                        "[ENTRY][BUYABLE_BACKFILL][PLAN_SKIP] code=%s reasons=%s",
                        _candidate_code(cf),
                        reasons,
                    )
                    return False
            ...
            _logger.info(
                "[ENTRY][BUYABLE_BACKFILL][PLAN_SKIP] code=%s err=%s",
                _candidate_code(cf),
                exc,
            )
            return False
...
def _apply_buyable_candidate_backfill(
    ...
    *,
    ...
    limit_candidates = [
        int(value or 0)
        for value in (new_position_limit, target_new_positions)
        if int(value or 0) > 0
    ]
    target_limit = (
        min(limit_candidates)
        if limit_candidates
        else max(1, _safe_int(os.getenv("PB1_BUYABLE_BACKFILL_MAX_CANDIDATES"), 1))
    )
    if target_limit <= 0:
        return 0
    ...
    _logger.info(
        ...
        len(pool),
        target_limit,
        sorted(blocked_codes),
    )
    ranked = sorted(
        pool,
        key=lambda cf: _safe_float(
            _candidate_features(cf).get("score_final")
            or _candidate_features(cf).get("final_score")
            or _candidate_features(cf).get("score")
        ),
        reverse=True,
    )
    ...
    if added:
        try:
            ...
            pass
        _logger.info(
            "[ENTRY][BUYABLE_BACKFILL][DONE] added=%s codes=%s",
            added,
            [_candidate_code(c) for c in orderable_candidates],
        )
    else:
...
def _wrap_pb1_engine_class(engine_cls: type[Any]) -> bool:
    """Install idempotent PB1 compatibility guards on the engine class."""
...
def _install_pb1_engine_runtime_guards() -> None:
    _ensure_engine_runner_fallback()
    try:
        from trader.pb1_engine import PB1Engine
    except Exception as exc:  # pragma: no cover - keep package import non-fatal
        _logger.warning("[PB1][ENGINE_RUNTIME_GUARD][INSTALL_DEFERRED] err=%s", exc)
        return
    installed = _wrap_pb1_engine_class(PB1Engine)
    if installed:
        _logger.info("[PB1][ENGINE_RUNTIME_GUARD][INSTALLED] target=PB1Engine")


# Backward-compatible old name used by early PR tests / ad hoc imports.
_install_pb1_engine_runner_finalization_guard = _install_pb1_engine_runtime_guards
