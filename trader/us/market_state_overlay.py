...
def _market_returns(provider: Any, trade_date: str, warnings: list[str]) -> dict[str, float | None]:
    ...
    for sym in ("SPY", "QQQ", "SMH", "DIA", "RSP", "XLK", "XLI", "XLF", "XLV", "XLP", "XLU", "XLE"):
        rows = None
        try:
...
def evaluate_us_market_state(*, trade_date: str, provider, rotation_context: dict | None, prep_result: dict | None, positions: list[dict] | None, account_snapshot: dict | None, now=None) -> dict:
    ...
    allow_new = state != "DEFENSE_CRASH" and acct["gross_exposure_pct"] < max_gross
    if acct["gross_exposure_pct"] >= max_gross: reasons.append("GROSS_EXPOSURE_CAP_REACHED")
    out = {**rets, **acct, "market_state": state, "defense_regime": state if state.startswith("DEFENSE") else "NONE", "risk_on_regime": state if state.endswith("RISK_ON") else "NONE", "market_state_reasons": reasons, "rotation_regime": rotation_regime, "exposure_multiplier": mults[state], "allow_new_buy": allow_new, "allow_add_to_existing": state in {"NORMAL", "RISK_ON", "STRONG_RISK_ON"}, "allow_ai_tech_buy": state in {"RISK_ON", "STRONG_RISK_ON"} and rotation_regime == "AI_ON", "allow_defensive_buy": state in {"DEFENSE_RISK_OFF", "DEFENSE_CAUTION", "NORMAL", "RISK_ON", "STRONG_RISK_ON"}, "force_entry_block": state == "DEFENSE_CRASH", "trim_required": state in {"DEFENSE_CRASH", "DEFENSE_RISK_OFF"}, "profit_capture_enabled": os.getenv("US_PROFIT_CAPTURE_ENABLE", "1") not in {"0", "false", "False"}, "trailing_stop_mode": modes[state], "trailing_stop_pct": trails[state], "trailing_stop_reason": f"market_state={state}", "stop_tightening_level": modes[state], "account_loss_kill_switch_triggered": loss, "account_loss_kill_switch_level": state if loss else "NONE", "account_pnl_source": "account_snapshot" if pnl1 is not None else "unknown", "account_intraday_pnl_pct_unknown": pnl1 is None, "data_quality": "degraded" if warnings or quality != "ok" else "ok", "data_quality_warnings": warnings, "forbidden_hedge_symbols": sorted(FORBIDDEN_HEDGE_SYMBOLS)}
