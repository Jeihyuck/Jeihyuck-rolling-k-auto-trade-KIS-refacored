...
def evaluate_portfolio_cluster_guard(positions: list[dict], regime: str, equity_usd: float, current_exit_intents: list[dict] | None = None, provider: Any | None = None, now: Any | None = None) -> dict:
    ...
    if over_cap and regime in ("RISK_OFF", "AI_OFF_ROTATION", "CONSERVATIVE_ROTATION"):
        ai_positions = [p for p in positions or [] if theme_cluster_for(str(p.get("symbol") or p.get("code") or ""), p) in AI_CLUSTERS]
    ...
    return {"portfolio_cluster_guard_status": status, "portfolio_ai_tech_weight": ai_weight, "portfolio_cluster_cap_violations": ["AI_TECH_COMBINED"] if over_cap else [], "blocked_clusters": sorted(blocked_clusters), "cluster_guard_trim_intents": trim_intents, "cluster_guard_trim_notional": trim_notional}
