...
def authoritative_broker_avg(position: dict, *, now: datetime | None = None,
    ...
    """Validate that TP cost basis is a fresh authoritative KIS USD value."""
    ...
    if position.get("balance_source") != "kis_balance_authoritative" or position.get("authoritative_positions") is not True:
        raise ValueError("missing_authoritative_broker_avg_price")
    if source not in AUTHORITATIVE_AVG_SOURCES or currency != "USD" or not asof_raw:
        raise ValueError("missing_authoritative_broker_avg_price")
    try:
    ...
    return as_decimal(position.get("broker_avg_price"), name="broker_avg_price"), {
