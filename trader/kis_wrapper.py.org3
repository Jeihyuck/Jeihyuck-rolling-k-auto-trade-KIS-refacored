...
class KisAPI:
    ...
    def _estimate_buy_cost(self, price: float, qty: int, fee_pct: float = 0.00015, tax_pct: float = 0.0) -> int:
        ...
        KOSDAQ 현물은 보통 매수 시 거래세 없음 → tax_pct=0.0 유지.
        """
        try:
            price = float(price)
        except Exception:
    ...
    def get_close_price(self, code: str) -> Optional[float]:
        ...
        # 스냅샷 fallback
        try:
            return float(self.get_current_price(code))
        except Exception:
            return None
    ...
    def get_quote_snapshot(self, code: str) -> Dict[str, Any]:
        ...
        out: Dict[str, Any] = {}
        try:
            out["tp"] = float(self.get_current_price(code))
        except Exception:
            out["tp"] = None
        try:
            out["ap"] = float(self.get_best_ask(code)) if self.get_best_ask(code) is not None else None
            out["bp"] = float(self.get_best_bid(code)) if self.get_best_bid(code) is not None else None
        except Exception:
    ...
    def get_index_quote(self, index_code: str) -> Dict[str, Optional[float]]:
        """
        (간이) 지수 스냅샷. KIS 인덱스 TR을 쓰지 않고, 현재는 빈 값 위주로 반환.
        필요 시 별도 인덱스 조회 TR로 교체 가능.
        """
        # 최소한의 필드만 제공(레짐 로직이 None을 안전 처리함)
        return {
            "price": None,
            "prev_close": None,
            "vwap": None,
    ...
    def buy_stock_market_guarded(self, code: str, qty: int, **kwargs):
        ...
        if ref_px <= 0:
            try:
                ...
                ref_px = float(snap.get("tp") or snap.get("trade_price") or snap.get("prpr") or 0.0)
            except Exception:
                ref_px = 0.0
