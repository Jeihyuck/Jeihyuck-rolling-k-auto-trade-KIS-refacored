...
class KISMarketcapTopProvider:
    ...
    # KIS official market_cap request contract.  Keep empty-string filters
    # explicit: the endpoint treats these fields as required even when the
    # caller wants the full price/volume range.
    DEFAULT_PARAMS = {
    ...
    # The market-cap endpoint uses J=KRX and separates KOSPI/KOSDAQ via
    ...
    def _build_params(self, market: str, n: int) -> dict:
        # n is kept in the provider API because callers request a target size;
        # the KIS market-cap endpoint itself does not accept FID_INPUT_CNT_1.
        # We slice the normalized response after the call instead of sending an
