...
# WSL cron must not run trading sessions under WSL. GitHub Actions are manual diagnostic/smoke/report only.
...
$defs = @(
  ...
  @{Name="PB1 US Prep Recovery WSL"; Time="22:10"; Cmd="bash $base/run-us-prep-recovery.sh"; Limit="PT1H"},
  @{Name="PB1 US AM Preflight WSL"; Time="22:20"; Cmd="bash $base/check-us-prep-before-am.sh"; Limit="PT30M"},
...
foreach ($d in $defs) {
  ...
  $settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1) -MultipleInstances IgnoreNew -ExecutionTimeLimit ([System.Xml.XmlConvert]::ToTimeSpan($d.Limit)) -WakeToRun
