...
# KR prep: 06:50 KST
50 6 * * 1-5 cd $APP && /usr/bin/flock -n /tmp/nullim-kr-prep.lock $APP/scripts/wsl/with-venv.sh $APP/scripts/wsl/run-kr-prep.sh >> $APP/runtime/cron/kr-prep.log 2>&1

# KR AM: 09:00 KST
0 9 * * 1-5 cd $APP && /usr/bin/flock -n /tmp/nullim-kr-am.lock $APP/scripts/wsl/with-venv.sh $APP/scripts/wsl/run-kr-am.sh >> $APP/runtime/cron/kr-am.log 2>&1

# KR afternoon: 13:30 KST
30 13 * * 1-5 cd $APP && /usr/bin/flock -n /tmp/nullim-kr-afternoon.lock $APP/scripts/wsl/with-venv.sh $APP/scripts/wsl/run-kr-afternoon.sh >> $APP/runtime/cron/kr-afternoon.log 2>&1

# KR close: 15:20 KST
20 15 * * 1-5 cd $APP && /usr/bin/flock -n /tmp/nullim-kr-close.lock $APP/scripts/wsl/with-venv.sh $APP/scripts/wsl/run-kr-close.sh >> $APP/runtime/cron/kr-close.log 2>&1
