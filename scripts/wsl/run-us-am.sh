...
if [[ "${US_LOCK_ONLY:-0}" == "1" ]]; then
  echo "[$(date -Is)] [US_WSL_LOCK][LOCK_ONLY_DONE] session=${SESSION_NAME} lock=${LOCK_FILE}" >> "$LOG_FILE"
  exit 0
fi
...
export US_OPENING_BUY_BLOCK_ENABLED="${US_OPENING_BUY_BLOCK_ENABLED:-1}"
export US_OPENING_BUY_BLOCK_MINUTES="${US_OPENING_BUY_BLOCK_MINUTES:-30}"
export US_MARKET_OPEN_ET="${US_MARKET_OPEN_ET:-09:30}"
export US_BUY_START_ET="${US_BUY_START_ET:-10:00}"
...
cmd=("${PYTHON_BIN}" -m trader.us.runner.trade_session_runner --session am --env practice --max-minutes "${max_minutes}" --interval-sec "${interval_sec}" --run-mode "${run_mode}")
