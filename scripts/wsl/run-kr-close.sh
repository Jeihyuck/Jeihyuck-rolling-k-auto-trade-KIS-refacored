...
LOG=runtime/wsl-kr-close.log
{
  echo "[KR_CLOSE][START] ts=$(date -Is) env=$STRATEGY_ENV kis_env=$KIS_ENV dry_run=$DRY_RUN live_enabled=$LIVE_TRADING_ENABLED kr_order_armed=$KR_ORDER_ARMED"
  set +e
  python - <<'PYGUARD'
...
if 'close' == 'am' and now.hour < 9:
    print('[KR_CLOSE][SKIP] reason=PREOPEN_NO_ORDER'); print('[RUN_SUMMARY][RESULT] status=SKIP reason=PREOPEN_NO_ORDER'); sys.exit(10)
...
for p in cands:
    if p.exists():
        ...
        if n>0:
            print('[KR_CLOSE][PREP_GUARD][OK] final30=%s source=%s' % (n,p)); sys.exit(0)
print('[KR_CLOSE][SKIP] reason=KR_PREP_ARTIFACT_MISSING'); print('[RUN_SUMMARY][RESULT] status=SKIP reason=KR_PREP_ARTIFACT_MISSING'); sys.exit(11)
PYGUARD
  guard=$?
  set -e
  if [[ $guard -eq 10 || $guard -eq 11 ]]; then echo "[KR_CLOSE][EXIT] ts=$(date -Is) exit_code=0 guard=$guard"; exit 0; fi
  echo "[KR_CLOSE][GATE] order_allowed=1"
  python -m trader.trader --window close --phase auto
  rc=$?
  echo "[KR_CLOSE][DONE] status=$([[ $rc -eq 0 ]] && echo OK || echo FAIL)"
  echo "[RUN_SUMMARY][RESULT] session=close status=$([[ $rc -eq 0 ]] && echo OK || echo FAIL)"
